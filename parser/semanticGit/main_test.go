...
func TestGitGood(t *testing.T) {
	...
	assert.Equal(t, "v0.0.0", gitSemantic.GetCurrentVersion().ToString(true))

	gitInMemory.TestCreateTag("v0.0.1", gitInMemory.TestCommit("fix: thing"))
	gitInMemory.TestCommit("feat: test2")

	assert.Equal(t, "v0.0.1", gitSemantic.GetCurrentVersion().ToString(true))

	assert.Equal(t, "v0.1.0", gitSemantic.CalculateNextVersion(gitSemantic.GetCurrentVersion()).ToString(true))

	gitInMemory.TestCreateTag("v0.1.0", gitInMemory.TestCommit("fix: thing"))

	gitInMemory.TestCommit("fix: test2")
	assert.Equal(t, "v0.1.1", gitSemantic.CalculateNextVersion(gitSemantic.GetCurrentVersion()).ToString(true))

	gitInMemory.TestCommit("feat: test2")

	assert.Equal(t, "v0.2.0", gitSemantic.CalculateNextVersion(gitSemantic.GetCurrentVersion()).ToString(true))
}
